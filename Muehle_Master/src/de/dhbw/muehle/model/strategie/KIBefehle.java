...
public class KIBefehle {
	...
	public void anzahlZugmoeglichkeitenschwarz(int Zugmoeglichkeitenschwarz) {
		// Hier m�ssen sp�ter alle M�glichen Zugm�glichkeiten !ProStein!
		...
		if (Zugmoeglichkeitenschwarz <= 10) {
			...
			System.out
					.println("Zugm�glichkeiten erfolglos durchlaufen schwarz");
		}
	}
	...
	public void anzahlZugmoeglichkeitenweiss(int Zugmoeglichkeitenweiss) {
		if (Zugmoeglichkeitenweiss <= 10) {
			...
			System.out.println("Zugm�glichkeiten erfolglos durchlaufen weiss");
		}
	}
...
//		anzahloffeneMuehlenweiss();
	...
	public void ermittleBestenZug() {
		if (core.getSpielphase() == 1) {
			// An dieser Stelle m�ssen keine Zugm�glichkeiten gepr�ft werden
			// Ebenfalls nicht die M�glicheBewegungsdifferenz
		...
		// for-Schleife, die jeden Spielstein durchl�uft und pr�ft, welche Z�ge
		// mit dem entsprechenden Stein
		// m�glich sind.
		// F�r jede m�gliche neue Position muss die aktuelleZugwertigkeit
		// berechnet werden
		// Hierf�r muss jede der Pr�fungsmethoden mit der neuen Funktion
		// durchlaufen werden
		// Die getestete Position wird in ein Array geschrieben (also der
		// Hashcode o.�. der die Position
		// eindeutig bestimmbar macht
		// In einem anderen Array muss am gleichen Index wie die Position die
		// Zugwertigkeit f�r den
	...
	public void ermittleBesteSetzmoeglichkeit() {
		...
		for (int z = 1; z <= 3; z++) {
			for (int x = 1; x <= 3; x++) {
				for (int y = 1; y <= 3; y++) {
					// �berpr�fe ob die Position der jeweiligen Z�hlvariablen
					// frei ist
					// �Berpr�fung kann durch die Hashcodes, die in STW und STS
					// gespeichert sind laufen
					// Man kann selbigen mit den Z�hlvariablen errechnen
					// �berpr�fe welche Wertigkeit an der aktuellen Position f�r
					// die Z�hlvariablen erreicht wird
				}
			}
		}
	}
	...
	public void pruefeZugmoeglichkeitenfueralleSteineweiss() {
		...
		for (int i = 0; (i < StonelistWhite.size()); i++) {
			...
			// Welche Z�ge f�r den einen Spielstein (i) m�glich sind
			...
			// Hashliste g�ltige Z�ge gesamt Weiss beinhaltet die Hashcodes aller
			// m�glichen Z�ge
			// Nun muss noch gepr�ft werden, welche dieser Felder durch andere
			// Spielsteine belegt sind
			// Die Informationen �ber alle anderen Spielsteine stehen in Stonelist
